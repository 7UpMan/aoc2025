...
public class Day02 {
    ...
    public Day02(List<String> inputLines) {
        ...
        inputLines.forEach(line -> 
            Arrays.asList(line.split(",")).forEach(part -> {
            ...
            for(long i = range.start; i <= range.end; i++) {
                ...
                for (int splitLen = 1; splitLen <= numberStrLen / 2 && !foundMatch; splitLen++) {
                    ...
                    for(int pos =1; pos < numberStrLen / splitLen; pos++) {
                        ...
                        // If we are at the end, we found a match
                        if(pos == (numberStrLen / splitLen) -1) {
                            resultsPart2 += i;
                            if (DEBUG) {
                                System.out.println("Found match for " + numberStr + " with part " + stringToLookFor);
                            }
                            
                            // Stop looking, 2222 us invlolved in multiple ways, only count once
                            foundMatch = true;
                        }
                    }
                }
